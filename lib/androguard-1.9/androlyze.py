...
option_9 = { 'name' : ('-a', '--activities'), 'help' : 'show activities', 'action' : 'count' }
option_10 = { 'name' : ('-z', '--zpackage'), 'help' : 'show package', 'action' : 'count' }
option_11 = { 'name' : ('-c', '--cservices'), 'help' : 'show services', 'action' : 'count' }
option_12 = { 'name' : ('-r', '--receivers'), 'help' : 'show receivers', 'action' : 'count' }
option_13 = { 'name' : ('-b', '--broviders'), 'help' : 'show providers', 'action' : 'count' }
option_14 = { 'name' : ('-e', '--easyperms'), 'help' : 'show list of permissions', 'action' : 'count' }
option_15 = { 'name' : ('-g', '--gmain'), 'help' : 'show main activity', 'action' : 'count' }
option_16 = { 'name' : ('-k', '--ksdk'), 'help' : 'show sdk version', 'action' : 'count' }
option_17 = { 'name' : ('-j', '--jactions'), 'help' : 'show list of actions', 'action' : 'count' }
option_18 = { 'name' : ('-l', '--lactivityactions'), 'help' : 'show list of activity actions', 'action' : 'count' }
option_19 = { 'name' : ('-n', '--ncategories'), 'help' : 'show list of categories', 'action' : 'count' }

options = [option_0, option_1, option_2, option_3, option_4, option_5, option_6, option_8, option_9, option_10, option_11, option_12, option_13, option_14, option_15, option_16, option_17, option_18, option_19]
...
def main(options, arguments) :
    if options.shell != None :
        ...
        elif options.activities != None :
            activities = _b.get_activities()
            for activity in activities :
                print activity
  
        elif options.zpackage != None :
            package = _b.get_package()
            print package
        
        elif options.cservices != None :
            services = _b.get_services()
            for service in services :
                print service
  
        elif options.receivers != None :
            receivers = _b.get_receivers()
            for receiver in receivers :
                print receiver
  
        elif options.broviders != None :
            providers = _b.get_providers()
            for provider in providers :
                print provider

        elif options.easyperms != None :
            easyperms = _b.get_details_permissions()
            for i in easyperms:
                print "(%s) %s: %s" % (string.ljust(easyperms[i][0],17), string.ljust(i,45), easyperms[i][2])
  
        elif options.gmain != None :
            print _b.get_main_activity()
  
        elif options.ksdk != None :
            print _b.get_min_sdk_version()
        ...
        elif options.jactions != None :
            actions = _b.get_actions()
            for action in actions:
                print action

        elif options.lactivityactions != None:
            activityactions = _b.get_activityactions()
            for activity, action in activityactions.iteritems():
                print '%s: %s' % (activity, action)

        elif options.ncategories != None:
            for category in _b.get_categories():
                print category
